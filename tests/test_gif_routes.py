...
class TestWaterGifRoutesSatellite:
    """Verifica el comportamiento de la ruta de cuerpos de agua con el parámetro satellite."""

    def test_water_gif_route_default_satellite_is_landsat(self, client, sample_dates):
        """Si no se especifica el satélite, debe usar 'landsat' por defecto."""
        mock_dir, _, mock_build_gif, mock_build_ts, mock_add = _make_cache_miss_mocks(sample_dates)

        with patch("routes.gif_routes.build_water_gif_bbox", mock_build_gif), \
             patch("routes.gif_routes.build_water_timeseries_bbox", mock_build_ts), \
             patch("routes.gif_routes.add_dates_to_gif", mock_add), \
             patch("routes.gif_routes.GIFS_DIR", mock_dir):

            client.get(f"/api/water-gif-bbox?start=2020-01-01&end=2020-12-31&bbox={SMALL_BBOX_URL}")

            mock_build_gif.assert_called_once_with("2020-01-01", "2020-12-31", [-92.0, 17.0, -91.0, 18.0], None, satellite="landsat")

    def test_water_gif_route_uses_specified_satellite(self, client, sample_dates):
        """Debe pasar el satélite especificado a la función de GEE."""
        mock_dir, _, mock_build_gif, mock_build_ts, mock_add = _make_cache_miss_mocks(sample_dates)
        ...
        with patch("routes.gif_routes.build_water_gif_bbox", mock_build_gif), \
            ...
            client.get(f"/api/water-gif-bbox?start=2020-01-01&end=2020-12-31&bbox={SMALL_BBOX_URL}&satellite=sentinel1")

            mock_build_gif.assert_called_once_with("2020-01-01", "2020-12-31", [-92.0, 17.0, -91.0, 18.0], None, satellite="sentinel1")

    def test_water_gif_route_rejects_invalid_satellite(self, client):
        """Debe retornar 400 si el satélite es inválido."""
        response = client.get(f"/api/water-gif-bbox?start=2020-01-01&end=2020-12-31&bbox={SMALL_BBOX_URL}&satellite=invalid")
        assert response.status_code == 400
        data = json.loads(response.get_data(as_text=True))
        assert "satellite debe ser" in data["error"]